...
class PhotoStore: ObservableObject {
    ...
    func setMainPhoto(_ image: UIImage) {
        // Save main photo to a special location
        let documentsPath = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask)[0]
        let mainPhotoURL = documentsPath.appendingPathComponent("main_photo.jpg")

        if let imageData = image.jpegData(compressionQuality: 0.8) {
            try? imageData.write(to: mainPhotoURL)
            print("PhotoStore: Main photo saved")
        }

        // Also add to regular images if not already there
...
struct TodayFullScreenView: View {
    ...
    var body: some View {
        ZStack {
            ...
            // Bottom overlay with quote (conditionally shown)
        ...
        VStack(alignment: .leading, spacing: 12) {
            ...
            if memoryDay.hasMemory {
                VStack(alignment: .leading, spacing: 10) {
                    ...
                    // Memory text - fetch fresh from HistoricalQuoteManager (conditionally shown)
                    if showQuotes {
                ...
                VStack(spacing: 30) {
                    ...
                    VStack(spacing: 20) {
                        ...
                        VStack(spacing: 15) {
                            ...
                            HStack {
                                VStack(alignment: .leading, spacing: 4) {
                                    Text("Show Daily Quotes")
                                        .font(.system(size: 16, weight: .medium, design: .serif))
                                        .foregroundColor(Color(red: 0.98, green: 0.97, blue: 0.95))

                                    Text("Display inspirational quotes with photos")
                                        .font(.system(size: 12, design: .serif))
                                        .foregroundColor(Color(red: 0.98, green: 0.97, blue: 0.95).opacity(0.7))
                                }
